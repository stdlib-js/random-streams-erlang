...
tape( 'the function throws an error if shape parameter `k` is not a positive integer', function test( t ) {
	var values;
	var i;
...
tape( 'the function throws an error if rate parameter `lambda` is not a positive number primitive', function test( t ) {
	var values;
	var i;
...
tape( 'attached to the returned stream is the underlying PRNG', function test( t ) {
	...
	t.strictEqual( typeof s.PRNG, 'function', 'has property' );

	s = randomStream( 2, 5.0, {
		'prng': minstd.normalized
	});
	t.strictEqual( s.PRNG, minstd.normalized, 'has property' );
...
tape( 'attached to the returned stream is the generator seed', function test( t ) {
	...
	t.strictEqual( isUint32Array( s.seed ), true, 'has property' );
	...
	t.strictEqual( s.seed, null, 'equal to `null`' );
...
tape( 'attached to the returned stream is the generator seed length', function test( t ) {
	...
	t.strictEqual( s.seedLength, null, 'equal to `null`' );
...
tape( 'attached to the returned stream is the generator state', function test( t ) {
	...
	t.strictEqual( isUint32Array( s.state ), true, 'has property' );

	s = randomStream( 2, 5.0, {
		'prng': minstd.normalized
	});
	t.strictEqual( s.state, null, 'equal to `null`' );
...
tape( 'attached to the returned stream is the generator state length', function test( t ) {
	...
	t.strictEqual( s.stateLength, null, 'equal to `null`' );
...
tape( 'attached to the returned stream is the generator state size', function test( t ) {
	...
	t.strictEqual( s.byteLength, null, 'equal to `null`' );

	t.end();
});

tape( 'the constructor returns a stream for generating pseudorandom numbers from an Erlang distribution', function test( t ) {
	var iStream;
	var result;
	var rand;
	var opts;
...
tape( 'the constructor returns a stream for generating pseudorandom numbers from an Erlang distribution (object mode)', function test( t ) {
	var iStream;
	var count;
	var rand;
	var opts;
...
tape( 'the constructor supports returning a seeded readable stream', function test( t ) {
	...
	s1 = randomStream( 2, 5.0, opts );
	s1.on( 'end', onEnd1 );

	s2 = randomStream( 2, 5.0, opts );
	s2.on( 'end', onEnd2 );
